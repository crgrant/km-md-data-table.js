...
function mdEditDialog($compile, $controller, $document, $mdUtil, $q, $rootScope, $templateCache, $templateRequest, $window) {
  ...
  function build(template, options) {
    ...
    if(options.clickOutsideToClose && options.saveItemOnClose) {
    ...
    else if (options.clickOutsideToClose && !options.saveItemOnClose){
      backdrop.on('click', function () {
        //dont save, only remove.
        element.remove();
      });
    }
